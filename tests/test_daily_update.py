...
def _game(home, away):
    return {"game_id": f"{away}@{home}", "home_team": home, "away_team": away, "status": "Scheduled"}
...
def isolated_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(daily_update.live, "get_injuries_batch", lambda game_ids: {})
...
def test_build_live_injury_lookup_skips_already_final_games():
    ...
    injuries_by_game = {"123": {"AAA": None}}
    final_game = {"game_id": "123", "status": "Final"}
    assert daily_update._build_live_injury_lookup(final_game, injuries_by_game) is None


def test_build_live_injury_lookup_works_for_a_not_yet_final_game():
    import pandas as pd

    report = pd.DataFrame([{"full_name": "Jaxson Dart", "position": "QB", "report_primary_injury": None, "report_status": "Out"}])
    injuries_by_game = {"123": {"NYG": report}}

    scheduled_game = {"game_id": "123", "status": "Scheduled"}
    lookup = daily_update._build_live_injury_lookup(scheduled_game, injuries_by_game)
